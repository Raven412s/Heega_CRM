...
export function DataTableRowActionsForProducts<TData>({
  row,
}: DataTableRowActionsProps<TData>) {
    ...
    const handleDelete = async () => {
      ...
      try {
        const response = await fetch(`/api/delete-product?_id=${row.original?._id}`, {
          method: 'DELETE',
        });
    ...
    const handleEdit = () => {
        router.push(`/superAdmin/products/edit?id=${row.original?._id}`);
      };
